...
read_absorption_separation <- function(file) {
  ...
  absorption_separation <- read_csv(file, col_types = col_types) |>
    rename_with(\(.) col_names, names(col_names)) |>
    relocate(date) |>
    arrange(date) |>
    mutate(
      across(
    ...
    relocate(city_code) |>
    mutate(
      pattern_city = str_glue("(\\((({city_name_kana})、)?({city_code})\\))"),
      pattern_city = case_when(
        city_name == "上九一色村" ~
          str_glue(
            "{city_name}{pattern_city}大字梯及び古関|大字精進、本栖及び富士ヶ嶺"
          ),

        # 2024-01-01: Kita-ku, Hamamatsu City
        city_desig_name == "浜松市" & city_name == "北区" ~
          str_glue(
            "{city_name}{pattern_city}(\\((三方原地区/初生町、三方原町、東三方町、豊岡町、三幸町、大原町、根洗町|三方原地区以外)\\))?"
          ),

        .default = str_glue(
          "({pref_name})?({city_desig_name})?{city_name}{pattern_city}?"
        )
      ),
      pattern_city = str_glue("^{pattern_city}$")
    ) |>
    nest(.by = c(date, event), .key = "city") |>
    mutate(
      pattern_city = city |>
        map_chr(\(city) {
          pattern_city <- city |>
  ...
  zero_or_more <- str_c(
    "{{",
    str_glue("0,{max(str_length(absorption_separation$event))}"),
    "}}"
  )
  pattern_city <- "{pattern_city}"
  pattern_cities <- str_glue(
    "({pattern_city}[、と]){zero_or_more}{pattern_city}"
  )
  ...
  pattern_city_to <- pattern_absorption_separation |>
    str_glue_data(
      "(?<={city_from}{verb_city_from}){city_to}(?={verb_city_to}$)"
    ) |>
    set_names(pattern_absorption_separation$name)
  ...
  absorption_separation |>
    ...
    mutate(
      type = case_when(
        str_detect(event, str_glue(pattern_type[["編入合併"]])) ~ "編入合併",
        str_detect(event, str_glue(pattern_type[["新設合併"]])) ~ "新設合併",
        str_detect(
          event,
          str_glue(pattern_type[["政令指定都市施行"]])
        ) ~ "政令指定都市施行",
        str_detect(
          event,
          str_glue(pattern_type[["区の新設|郡の新設"]])
        ) ~ "区の新設|郡の新設",
        ...
        str_detect(
          event,
          str_glue(pattern_type[["郡の区域変更"]])
        ) ~ "郡の区域変更",
        str_detect(event, str_glue(pattern_type[["郡の廃止"]])) ~ "郡の廃止"
      )
    ) |>
    nest(.by = type, .key = "absorption_separation") |>
    left_join(
      pattern_city_from |>
    ...
    mutate(
      absorption_separation = list(
        ...
        pmap(\(absorption_separation, pattern_city_from, pattern_city_to) {
          absorption_separation |>
            mutate(
              pattern_city_from = str_glue(pattern_city_from),
    ...
    unnest(city_from, keep_empty = TRUE) |>
    unnest(city_to, keep_empty = TRUE) |>
    mutate(
      from = list(city, city_from) |>
