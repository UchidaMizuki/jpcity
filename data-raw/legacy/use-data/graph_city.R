...
get_graph_city <- function(
  ...
  absorption_separation <- absorption_separation |>
    filter(
      date != "2005-03-01" |
        is.na(from$city_code) |
        from$city_code != "17421" |
        to$city_code == "17461",
      date != "2005-03-01" |
        is.na(from$city_code) |
        from$city_code != "17422" |
        to$city_code == "17462"
    )
  ...
  areacode_end <- tidy_city(areacode_end) |>
    ...
    filter(is.na(pref_name) | !is.na(city_desig_name) | !is.na(city_name))
  absorption_separation <- absorption_separation |>
    mutate(across(c(from, to), tidy_city)) |>
    vec_unique() |>
    filter(
      is.na(from$pref_name) |
        !is.na(from$city_desig_name) |
        !is.na(from$city_name),
      is.na(to$pref_name) | !is.na(to$city_desig_name) | !is.na(to$city_name)
    )

  nodes_city <- bind_rows(
    vec_init(areacode_start),
    areacode_start,
  ...
  edges_city <- bind_rows(
    areacode_start |>
      ...
      add_column(date = int_start(interval_city), type = "start", from = 1L),
    absorption_separation |>
      mutate(
        from = from |>
          left_join(nodes_city, by = col_names_city) |>
          pull(node),
        to = to |>
          left_join(nodes_city, by = col_names_city) |>
          pull(node)
    ...
    areacode_end |>
      ...
      add_column(
        date = as.POSIXct(Inf), # int_end(interval_city) + days(1L),
        to = 1L
      )
  ) |>
  ...
  date_start <- edges_city |>
    summarise(date_start = min(date), .by = to)
  date_end <- edges_city |>
    summarise(date_end = max(date) - days(1L), .by = from)

  graph_city <- graph_city |>
    mutate(node = row_number()) |>
    left_join(date_start, by = join_by(node == to)) |>
    left_join(date_end, by = join_by(node == from)) |>
    select(!node) |>
    mutate(
      interval = if_else(
        is.na(city_code),
        interval(NA_Date_, NA_Date_, tzone = tz_jst_legacy),
        date_start %--% date_end
      )
    ) |>
      ...
      ancestors = node |>
        map(
          \(node) {
            ...
            int_end(vec_slice(
              nodes_ancestors$interval,
              edges_ancestors$from
            )) <- edges_ancestors$date - days(1L)
      ...
      descendants = node |>
        map(
          \(node) {
            ...
            int_start(vec_slice(
              nodes_descendants$interval,
              edges_descendants$to
            )) <- edges_descendants$date
  ...
  nodes_city <- nodes_city |>
    ...
    mutate(across(
      c(city_desig_name, city_desig_name_kana, city_name, city_name_kana),
      \(x) replace_na(x, "")
    ))
...
tidy_city <- function(data) {
  data |>
    mutate(
      city_desig_name = if_else(
        str_ends(city_desig_name, "市"),
        city_desig_name,
        NA_character_
      ),
      city_desig_name_kana = if_else(
        str_ends(city_desig_name, "市"),
        city_desig_name_kana,
        NA_character_
      ),
      city_name_kana = case_match(
        city_code,
        "02445" ~ "なんぶちょう",
        "41401" ~ "ありたちょう",
