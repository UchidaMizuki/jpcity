city_legacy <- function(data, interval) {
  new_rcrd(
    data[c(
      "city_code",
      "pref_name",
...
#'
#' @export
city_data_legacy <- function(city) {
  data_frame(
    city_code = field(city, "city_code"),
...
check_city_interval_legacy <- function(
  city_code,
  interval,
  message_when = FALSE
) {
  out <- intersect_interval_legacy(interval)

  if (!all(is.na(interval)) && is.na(out)) {
    data <- data_frame(city_code = city_code, interval = interval) |>
      dplyr::mutate(
        date_start = lubridate::date(lubridate::int_start(interval)),
        date_end = lubridate::date(lubridate::int_end(interval))
      )
    oldest <- data |>
      dplyr::slice_min(.data$date_end, n = 1L)
    newest <- data |>
      dplyr::slice_max(.data$date_start, n = 1L)
    message <- c(
      "Intervals of {.arg city_code} must overlap.",
    ...
    if (message_when) {
      message <- c(message, "i" = "The {.arg when} argument must be given.")
    }
...
vec_restore.jpcity_city <- function(x, to, ...) {
  if (vec_is_empty(x)) {
    ...
    out <- add_city_data_legacy(x)
    city_legacy(
      out,
      interval = check_city_interval_legacy(out$city_code, out$interval)
    )
  }
}
